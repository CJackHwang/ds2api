...
- `internal/config`：配置加载、校验、运行时 settings 热更新；包含两个 feature flag：`context_engine.mode`（`off`/`shadow`/`enforce`，环境变量 `DS2API_CONTEXT_ENGINE`）和 `parser_v2.mode`（`off`/`shadow`/`enforce`，环境变量 `DS2API_PARSER_V2`）。详见 `docs/toolcall-semantics.md`（parser_v2）和 `docs/dev-plan-context-engine.md`（context_engine）。
