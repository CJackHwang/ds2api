...
Chuyển đổi khả năng hội thoại của DeepSeek Web thành API tương thích với OpenAI, Claude và Gemini. Phần backend cốt lõi được viết bằng **Go**; phần cầu nối stream trên Vercel sử dụng thêm một lượng nhỏ Node Runtime; frontend là React WebUI dùng làm trang quản trị, mã nguồn nằm trong `webui/` và khi deploy sẽ tự động build vào `static/admin`.

Lối vào tài liệu: [Điều hướng tài liệu](docs/README.md) / [Mô tả kiến trúc](docs/ARCHITECTURE.md) / [Tài liệu giao diện](API.md)

【Cảm ơn cộng đồng Linux.do và các lập trình viên trong cộng đồng GitHub đã hỗ trợ, đóng góp cho dự án】
...
>
> Kho lưu trữ này chỉ dùng cho mục đích học tập, nghiên cứu, thử nghiệm cá nhân và xác minh nội bộ. Dự án không cung cấp bất kỳ hình thức cấp phép thương mại, bảo đảm tính phù hợp hay bảo đảm kết quả nào.
>
> Tác giả và người duy trì kho lưu trữ không chịu trách nhiệm đối với bất kỳ thiệt hại trực tiếp hoặc gián tiếp nào phát sinh từ việc sử dụng, chỉnh sửa, phân phối, deploy hoặc phụ thuộc vào dự án này, bao gồm nhưng không giới hạn ở việc khóa tài khoản, mất dữ liệu, rủi ro pháp lý hoặc khiếu nại từ bên thứ ba.
>
> Không sử dụng dự án này trong các trường hợp vi phạm điều khoản dịch vụ, thỏa thuận, quy định pháp luật hoặc quy tắc nền tảng. Trước khi sử dụng cho mục đích thương mại, vui lòng tự xác nhận `LICENSE`, các thỏa thuận liên quan và việc bạn đã có văn bản cho phép từ tác giả hay chưa.
...
- [Bắt đầu nhanh](#bắt-đầu-nhanh)
  ...
  - [Cách 4: Chạy source code cục bộ](#cách-4-chạy-source-code-cục-bộ)
...
- [Công cụ bắt gói khi phát triển cục bộ](#công-cụ-bắt-gói-khi-phát-triển-cục-bộ)
...
Chi tiết phân tách kiến trúc và trách nhiệm thư mục xem tại [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
| CORS thống nhất | `/v1/*`, `/anthropic/*`, `/v1beta/models/*`, `/admin/*` dùng chung một bộ CORS strategy; trên Vercel, Node Runtime của `/v1/chat/completions` cũng đồng bộ cùng quy tắc cho phép, nhằm giảm tối đa hạn chế preflight request header từ client bên thứ ba |
...
OpenAI `/v1/*` vẫn là path chuẩn được khuyến nghị. Đồng thời dự án cũng hỗ trợ các route rút gọn ở root như `/models`, `/chat/completions`, `/responses`, `/embeddings`, `/files`, `/files/{file_id}` để thuận tiện cho client bên thứ ba chỉ cấu hình root address của DS2API.
...
Ngoài model gốc, dự án cũng hỗ trợ các alias phổ biến như `gpt-4.1`, `gpt-5`, `gpt-5-codex`, `o3`, `claude-*`, `gemini-*`, v.v. Tuy nhiên, `/v1/models` chỉ trả về DeepSeek native model ID đã được chuẩn hóa. Nếu alias tự nó có hậu tố `-nothinking`, alias đó cũng sẽ được ánh xạ sang model tương ứng có ngữ nghĩa bắt buộc tắt thinking. Hành vi alias đầy đủ lấy [API.md](API.md#模型-alias-解析策略) và `config.example.json` làm chuẩn.
...
- Nếu gặp lỗi “tool call xuất ra dạng text, không thực thi”, hãy ưu tiên kiểm tra model output có phải là DSML tool block được khuyến nghị không: `<|DSML|tool_calls><|DSML|invoke name="..."><|DSML|parameter name="...">...`. Lớp tương thích cũng chấp nhận canonical XML cũ: `<tool_calls><invoke name="..."><parameter name="...">...`. Các dạng cũ như `<tools>` / `<tool_call>` / `<tool_name>` / `<param>`, `<function_call>`, `tool_use` hoặc đoạn JSON thuần `tool_calls` sẽ không được thực thi.

### Giao diện Gemini

Gemini adapter ánh xạ tên model sang DeepSeek native model thông qua `model_aliases` hoặc rule tích hợp sẵn. Adapter hỗ trợ hai cách gọi `generateContent` và `streamGenerateContent`, đồng thời hỗ trợ đầy đủ Tool Calling, chuyển `functionDeclarations` thành output `functionCall`. Nếu tên Gemini model có hậu tố `-nothinking`, ví dụ `gemini-2.5-pro-nothinking`, model đó sẽ được ánh xạ sang model tương ứng bắt buộc tắt thinking.
...
> **Mô tả stream**: Trên Vercel, `/v1/chat/completions` mặc định đi qua `api/chat-stream.js` bằng Node Runtime để bảo đảm SSE thời gian thực. Xác thực, chọn tài khoản, chuẩn bị session/PoW vẫn do internal prepare interface của Go hoàn thành; stream response, bao gồm `tools`, được xử lý ở phía Node với logic lắp ráp output và chống rò rỉ đồng bộ với Go. Dù chỉ OpenAI chat stream dùng Node, CORS allow strategy vẫn giữ đồng bộ với Go main router, bao phủ thống nhất các tình huống preflight của client bên thứ ba.

Chi tiết deploy xem tại [Hướng dẫn deploy](docs/DEPLOY.md).

### Cách 4: Chạy source code cục bộ

**Yêu cầu trước**: Go 1.26+, Node.js `20.19+` hoặc `22.12+` nếu cần build WebUI; đồng thời bảo đảm có `npm`, khuyến nghị `npm 10+`.
...
> **WebUI tự động build**: Khi khởi động local lần đầu, nếu `static/admin` chưa tồn tại, chương trình sẽ tự động thử chạy `npm ci` nếu thiếu dependency và `npm run build -- --outDir static/admin --emptyOutDir`, yêu cầu máy local có Node.js và npm. Bạn cũng có thể build thủ công bằng:
>
> ```bash
...
Nếu tài khoản được chỉ định không tồn tại, hoặc hàng đợi của tài khoản quản trị hiện tại đã đầy, request sẽ trả về `429`. Hiện tại `429` không kèm header `Retry-After`. Nếu tài khoản tồn tại nhưng đăng nhập hoặc refresh thất bại, hệ thống sẽ trả về lỗi xác thực tương ứng.
...
2. Parser hiện xem DSML wrapper là dạng gọi thực thi được khuyến nghị: `<|DSML|tool_calls>` → `<|DSML|invoke name="...">` → `<|DSML|parameter name="...">`; đồng thời tương thích canonical XML cũ: `<tool_calls>` → `<invoke name="...">` → `<parameter name="...">`. DSML chỉ là alias lớp vỏ; bên trong vẫn phân tích ngữ nghĩa theo XML. Các dạng cũ như `<tools>` / `<tool_call>` / `<tool_name>` / `<param>`, `<function_call>`, biến thể `tool_use` / antml và đoạn JSON thuần `tool_calls` đều sẽ được xử lý như văn bản bình thường.
...
## Công cụ bắt gói khi phát triển cục bộ
