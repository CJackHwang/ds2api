...
6. strict 模式下只要片段中形成可解析的完整 tool JSON（即使前后混有 prose），仍会拦截并输出 tool_calls；仅保留前后自然语言文本。
7. 对工具参数中的路径字段（如 `path`/`file`/`cwd`）采用保守反斜杠修复，避免 `C:\new\tmp` 被污染为换行/Tab，同时非路径文本字段保持原始 `\n` 语义。
