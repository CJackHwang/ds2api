...
func TestHandleVercelStreamSwitchReuploadsCurrentInputFile(t *testing.T) {
	...
	testHistoryFilename := "test-history-reupload.txt"
	testToolsFilename := "test-tools-reupload.txt"
	stdReq := promptcompat.StandardRequest{
		RequestedModel:          "deepseek-v4-flash",
		ResolvedModel:           "deepseek-v4-flash",
		ResponseModel:           "deepseek-v4-flash",
		FinalPrompt:             "Continue from the latest state in the attached " + testHistoryFilename + " context. Available tool descriptions and parameter schemas are attached in " + testToolsFilename + "; use only those tools and follow the tool-call format rules in this prompt.",
		PromptTokenText:         "# " + testHistoryFilename + "\n\n=== 1. USER ===\nhello\n\n# " + testToolsFilename + "\nAvailable tool descriptions and parameter schemas for this request.\n\nYou have access to these tools:\n\nTool: search\nDescription: search docs\nParameters: {\"type\":\"object\"}\n",
		HistoryText:             "# " + testHistoryFilename + "\n\n=== 1. USER ===\nhello\n",
		CurrentInputFileApplied: true,
		CurrentInputFileID:      "file-old",
		CurrentToolsFileID:      "file-old-tools",
		HistoryFilename:         testHistoryFilename,
		ToolsFilename:           testToolsFilename,
		ToolsRaw: []any{
			map[string]any{
				"type": "function",
				"function": map[string]any{
	...
	reuploadHistoryFilename := ds.uploadCalls[0].Filename
	reuploadToolsFilename := ds.uploadCalls[1].Filename
	if !strings.HasSuffix(reuploadHistoryFilename, ".txt") || len(reuploadHistoryFilename) < 10 {
		t.Fatalf("unexpected reupload history filename: %q", reuploadHistoryFilename)
	}
	if !strings.HasSuffix(reuploadToolsFilename, ".txt") || len(reuploadToolsFilename) < 10 {
		t.Fatalf("unexpected reupload tools filename: %q", reuploadToolsFilename)
	}
	var body map[string]any
	if err := json.NewDecoder(rec.Body).Decode(&body); err != nil {
		t.Fatalf("decode failed: %v", err)
	...
	promptText, _ := payload["prompt"].(string)
	if !strings.Contains(promptText, reuploadToolsFilename) {
