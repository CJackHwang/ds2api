...
func TestBuildToolCallInstructions_ExecCommandUsesCmdExample(t *testing.T) {
	...
	if !strings.Contains(out, `<|DSML|parameter name="cmd" string="true">pwd</|DSML|parameter>`) {
...
func TestBuildToolCallInstructions_ExecuteCommandUsesCommandExample(t *testing.T) {
	...
	if !strings.Contains(out, `<|DSML|parameter name="command" string="true">pwd</|DSML|parameter>`) {
...
func TestBuildToolCallInstructions_BashUsesCommandAndDescriptionExamples(t *testing.T) {
	...
	for _, block := range blocks {
		if !strings.Contains(block, `<|DSML|parameter name="command" string="true">`) {
		...
		if strings.Contains(block, `<|DSML|parameter name="description" string="true">`) {
			sawDescription = true
		}
	}
	if !sawDescription {
...
func TestBuildToolCallInstructions_ExecCommandLongScriptUsesCmd(t *testing.T) {
	...
	for _, block := range blocks {
		if !strings.Contains(block, `<|DSML|parameter name="cmd" string="true">`) {
...
func TestBuildToolCallInstructions_WriteUsesFilePathAndContent(t *testing.T) {
	...
	for _, block := range blocks {
		if !strings.Contains(block, `<|DSML|parameter name="file_path" string="true">`) || !strings.Contains(block, `<|DSML|parameter name="content" string="true">`) {
